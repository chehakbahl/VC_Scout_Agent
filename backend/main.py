...
def scout(data: RequestData):
    ...
    s_score, s_reasons = spam_score(data.goal)
    i_score, i_reasons = injection_risk_score(data.goal)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
    try:
...
def monitor(data: MonitorRequest):
    ...
    s_score, s_reasons = spam_score(data.query)
    i_score, i_reasons = injection_risk_score(data.query)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
...
def trend(data: TopicRequest):
    ...
    s_score, s_reasons = spam_score(data.topic)
    i_score, i_reasons = injection_risk_score(data.topic)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
...
def discover(data: SectorRequest):
    ...
    s_score, s_reasons = spam_score(data.sector)
    i_score, i_reasons = injection_risk_score(data.sector)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
...
def interview(data: FounderRequest):
    ...
    s_score, s_reasons = spam_score(data.founder)
    i_score, i_reasons = injection_risk_score(data.founder)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
...
def memo(data: CompanyRequest):
    ...
    s_score, s_reasons = spam_score(data.company)
    i_score, i_reasons = injection_risk_score(data.company)
    if i_score >= INJECTION_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (injection risk {i_score}): {', '.join(i_reasons) or 'high risk'}")
    if s_score >= SPAM_THRESHOLD:
        raise HTTPException(status_code=400, detail=f"Blocked input (spam score {s_score}): {', '.join(s_reasons) or 'high risk'}")
